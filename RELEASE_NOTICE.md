...
>
> The automatic configuration upgrader reads and writes only `config.yml` and timestamped local migration backups inside the WRTPKILL plugin directory. The obsolete `.wrtpkill-default-config.yml` baseline is deleted, backups remain on the server, and none of these files are uploaded.
>
...
>
> 自动配置升级器只会读写 WRTPKILL 插件目录内的 `config.yml` 和带时间戳的本地迁移备份；已废弃的 `.wrtpkill-default-config.yml` 基线会被删除，备份只保存在服务器上，这些文件都不会被上传。
